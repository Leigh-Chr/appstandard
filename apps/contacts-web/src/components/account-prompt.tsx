...
import { Book, Cloud, Smartphone, X } from "lucide-react";
...
export function AccountPrompt({
	...
	const isNearLimit =
		usage &&
		(usage.addressBookCount >= usage.maxAddressBooks * 0.8 ||
			Object.values(usage.contactCounts).some(
				(count: number) => count >= usage.maxContactsPerAddressBook * 0.8,
			));
				...
				<div className="flex items-center gap-3">
					...
					<div className="min-w-0">
						<p className="font-medium text-sm">
							{isNearLimit ? (
								<>
									You're approaching the limit ({usage?.addressBookCount}/
									{usage?.maxAddressBooks} address books)
								</>
							) : (
								"Sync your contacts across all your devices"
							)}
						</p>
						<p className="text-muted-foreground text-xs">
							{isNearLimit
								? "Create a free account for up to 100 address books"
		...
		<Card className="mb-4">
			<CardHeader className="pb-3">
				<div className="flex items-start justify-between">
					<div>
						...
						<CardDescription>
							Back up your address books in the cloud
						</CardDescription>
					</div>
					{dismissible && (
						<Button
			...
			<CardContent className="space-y-4">
				...
				{isNearLimit && usage && (
					<div className="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800/50 dark:bg-amber-900/10">
						<p className="font-medium text-amber-700 text-sm dark:text-amber-400">
							{usage.addressBookCount}/{usage.maxAddressBooks} address books
							used
						</p>
					</div>
				)}
				...
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
					...
					<BenefitItem icon={Book} label="100 address books" />
				</div>
