...
function UsageDetails({ usage }: { usage: UsageInfo }) {
	const addressBookPercentage = Math.round(
		(usage.addressBookCount / usage.maxAddressBooks) * 100,
	);
		...
		<>
			<div className="space-y-2">
				<div className="flex items-center justify-between text-sm">
					<span className="text-muted-foreground">Address books</span>
					<span className="font-medium">
						{usage.addressBookCount} / {usage.maxAddressBooks}
					</span>
				</div>
				<Progress value={addressBookPercentage} className="h-2" />
			</div>
			<div className="space-y-2">
				<div className="flex items-center justify-between text-sm">
					<span className="text-muted-foreground">Contacts per book</span>
					<span className="font-medium">
						Max {usage.maxContactsPerBook.toLocaleString("en-US")}
					</span>
				</div>
			</div>
		</>
