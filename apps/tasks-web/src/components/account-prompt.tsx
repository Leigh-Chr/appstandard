...
import { CheckSquare, Cloud, Smartphone, X } from "lucide-react";
...
export function AccountPrompt({
	...
	const isNearLimit =
		usage &&
		(usage.taskListCount >= usage.maxTaskLists * 0.8 ||
			Object.values(usage.taskCounts).some(
				(count: number) => count >= usage.maxTasksPerTaskList * 0.8,
			));
				...
				<div className="flex items-center gap-3">
					...
					<div className="min-w-0">
						<p className="font-medium text-sm">
							{isNearLimit ? (
								<>
									You're approaching the limit ({usage?.taskListCount}/
									{usage?.maxTaskLists} task lists)
								</>
							) : (
						...
						<p className="text-muted-foreground text-xs">
							{isNearLimit
								? "Create a free account for up to 100 task lists"
		...
		<Card className="mb-4">
			<CardHeader className="pb-3">
				<div className="flex items-start justify-between">
					<div>
						...
						<CardDescription>
							Back up your task lists in the cloud
						</CardDescription>
					</div>
					{dismissible && (
						<Button
			...
			<CardContent className="space-y-4">
				...
				{isNearLimit && usage && (
					<div className="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800/50 dark:bg-amber-900/10">
						<p className="font-medium text-amber-700 text-sm dark:text-amber-400">
							{usage.taskListCount}/{usage.maxTaskLists} task lists used
						</p>
					</div>
				)}
				...
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
					...
					<BenefitItem icon={CheckSquare} label="100 task lists" />
				</div>
