 ...
 * Reusable search and sort components for task lists
 */
...
export type TaskListSortBy = "name" | "updatedAt" | "createdAt" | "taskCount";
...
interface SearchSortBarProps {
	...
	onSortChange: (sortBy: TaskListSortBy) => void;
	onSortDirectionChange?: (sortDirection: TaskListSortDirection) => void;
	/** Show sort direction toggle (only for date-based sorts) */
	showDirectionToggle?: boolean;
	/** Custom sort options (defaults to all options) */
	sortOptions?: Array<{ value: TaskListSortBy; label: string }>;
}

const DEFAULT_SORT_OPTIONS: Array<{ value: TaskListSortBy; label: string }> = [
...
export function TaskListSearchSortBar({
	keyword,
	sortBy,
	sortDirection = "desc",
	onKeywordChange,
	...
	return (
		...
		>
			...
			<div className={isMobile ? "flex gap-2" : "flex items-center gap-2"}>
				<Select
					value={sortBy}
					onValueChange={(v) => onSortChange(v as TaskListSortBy)}
				>
