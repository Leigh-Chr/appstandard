...
import { TasksCreateGroupDialog } from "./task-list-view/create-group-dialog";
import { TasksTour, useTasksTour } from "./tour";
...
export function GlobalFeatures() {
	...
	const shortcuts = useMemo(() => {
		...
		const common = buildCommonShortcuts({
			onNew: () => navigate({ to: "/tasks/new" }),
			onGoToList: () => navigate({ to: "/tasks" }),
			onGoToDashboard: () => navigate({ to: "/dashboard" }),
			onGoToAccount: () => navigate({ to: "/account" }),
			onImport: () => navigate({ to: "/tasks/import" }),
			onMerge: () => navigate({ to: "/tasks/merge" }),
			onSearch: () => {
	...
	return (
		<>
			...
			<KeyboardShortcutsDialog
				...
				description="Use these shortcuts to navigate AppStandard Tasks faster."
			/>
			<TasksCreateGroupDialog
