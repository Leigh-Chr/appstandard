...
type CommandAction = {
	...
	action: () => void;
	keywords?: string[];
	group: "navigation" | "actions" | "taskLists" | "settings";
};
...
export function CommandPalette({
	...
	const commands: CommandAction[] = useMemo(
		() => [
			...
			// Actions
			{
				...
				shortcut: "N",
				action: () => closeAndNavigate("/tasks/new"),
				keywords: ["new", "create", "task", "list"],
				group: "actions",
			},
			{
				id: "import",
				...
				shortcut: "I",
				action: () => closeAndNavigate("/tasks/import"),
				keywords: ["import", "ics", "file", "upload"],
				group: "actions",
			},
			{
				id: "merge",
				label: "Merge task lists",
				icon: <GitMerge className="h-4 w-4" />,
				shortcut: "M",
				action: () => closeAndNavigate("/tasks/merge"),
				keywords: ["merge", "combine", "tasks"],
				group: "actions",
			},
			{
				id: "create-group",
			...
			...taskLists.map((taskList) => ({
				...
				icon: (
					<div
						...
						style={{ backgroundColor: taskList.color || "#10B981" }}
					/>
				),
				action: () => closeAndNavigate(`/tasks/${taskList.id}`),
				keywords: [taskList.name.toLowerCase(), "task", "list"],
				group: "taskLists" as const,
			})),
		],
		[
			taskLists,
	...
	const taskListCommands = commands.filter((c) => c.group === "taskLists");
	...
	return (
		<CommandDialog open={open} onOpenChange={onOpenChange}>
			...
			<CommandList>
				...
				{/* Task Lists */}
				{taskListCommands.length > 0 && (
					<>
						<CommandSeparator />
						<CommandGroup heading="Task Lists">
							{taskListCommands.map((command) => (
								<CommandItem
									key={command.id}
									value={`${command.label} ${command.keywords?.join(" ")}`}
									onSelect={command.action}
