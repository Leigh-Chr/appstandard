...
export const Route = createFileRoute("/tasks/new")({
	...
	head: () => ({
		meta: [
			{ title: "Create a task list - AppStandard Tasks" },
			{
				name: "description",
				content:
					"Create a new task list in seconds. Organize your tasks and stay productive.",
			},
		],
	}),
...
function NewTaskListPage() {
	...
	const [color, setColor] = useState<string | null>("#22c55e");
	const { createTaskList, isCreating } = useCreateTaskList();
	...
	const handleCreate = () => {
		...
		createTaskList(
			...
			{
				onSuccess: (taskList) => {
					toast.success("Task list created! Start adding tasks.");
					navigate({ to: `/tasks/${taskList.id}` });
				},
				onError: (error) => {
			...
			<div className="container mx-auto max-w-2xl px-4 py-6 sm:py-10">
				<Card className="card-glow">
					<CardHeader>
						...
						<CardDescription>
							Create an empty task list to start adding tasks
						</CardDescription>
					</CardHeader>
